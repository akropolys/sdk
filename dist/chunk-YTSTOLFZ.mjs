...
var HuskelAPI = class {
  ...
  async chat(query, history = []) {
    ...
    const path = this.vertical === "property" ? "/chat/property" : "/chat";
  ...
  async chatStream(query, history = [], signal) {
    ...
    const path = this.vertical === "property" ? "/chat/stream/property" : "/chat/stream";
...
function mapRawProduct(input) {
  ...
  return {
    ...
    listing_price: typeof input.listing_price === "string" ? parseFloat(input.listing_price) : input.listing_price,
    price_upon_request: input.price_upon_request,
    mls_id: input.mls_id,
    listing_tier: input.listing_tier,
    listing_url: input.listing_url,
    listing_last_updated: input.listing_last_updated,
    ...
    full_bathrooms: input.full_bathrooms,
    half_bathrooms: input.half_bathrooms,
    living_area_sqft: input.living_area_sqft,
    ...
    garage_spaces: input.garage_spaces,
    parking: input.parking,
    flooring: input.flooring,
    appliances: input.appliances,
    interior_features: input.interior_features,
    water_frontage: input.water_frontage,
    water_source: input.water_source,
    sewer: input.sewer,
    heat_type: input.heat_type,
    air_conditioning: input.air_conditioning,
    utilities: input.utilities,
    real_estate_tax_annual: typeof input.real_estate_tax_annual === "string" ? parseFloat(input.real_estate_tax_annual) : input.real_estate_tax_annual,
    hoa_fees_monthly: typeof input.hoa_fees_monthly === "string" ? parseFloat(input.hoa_fees_monthly) : input.hoa_fees_monthly,
    virtual_tour_url: input.virtual_tour_url,
    listing_agent_name: input.listing_agent_name,
    listing_agent_phone: input.listing_agent_phone,
    listing_agent_email: input.listing_agent_email,
    co_agent_name: input.co_agent_name,
    brokerage: input.brokerage,
    bathrooms: input.bathrooms,
    living_area_sqm: input.living_area_sqm,
    total_area_sqm: input.total_area_sqm,
    land_size: input.land_size,
    land_size_unit: input.land_size_unit,
    date_listed: input.date_listed,
    county: input.county,
    country: input.country,
    district: input.district,
    sub_county: input.sub_county,
    tenure: input.tenure,
    listing_agent_photo: input.listing_agent_photo,
    listing_agent_whatsapp: input.listing_agent_whatsapp,
    listing_agent_license: input.listing_agent_license
  };
}
...
function HuskelProvider({ siteId, apiUrl, apiToken, shopperId, vertical, children }) {
  const clientRef = useRef2(null);
  if (!clientRef.current) {
    clientRef.current = new HuskelClient({ siteId, apiUrl, apiToken, shopperId, vertical });
  } else {
...
function useIngest() {
  ...
  const ingestBatch = useCallback2((products) => {
    ...
    client.queueIngestBatch(products).catch(() => {
    });
  }, [client]);
...
import { useState as useState2, useCallback as useCallback3, useRef as useRef5 } from "react";
...
import { useState as useState3, useEffect as useEffect3, useCallback as useCallback4 } from "react";
function useCart() {
  const client = useHuskelContext();
...
import { useState as useState4, useEffect as useEffect4, useRef as useRef6 } from "react";
...
import { useState as useState5, useEffect as useEffect5, useRef as useRef7 } from "react";
...
import { useState as useState7, useRef as useRef9, useEffect as useEffect7 } from "react";
...
import { useState as useState8, useEffect as useEffect8, useRef as useRef10, useCallback as useCallback5 } from "react";
    ...
    /* @__PURE__ */ jsx7("div", { className: "hsk-cb-sources", ref: railRef, children: sources.map((src, si) => {
      ...
      return /* @__PURE__ */ jsxs5(
        "div",
        {
          ...
          children: [
            src.image ? /* @__PURE__ */ jsxs5("div", { className: "hsk-cb-src-imgwrap", style: { position: "relative" }, children: [
              ...
              }, children: /* @__PURE__ */ jsx7(SparkleIcon3, { size: 12 }) })
            ] }) : /* @__PURE__ */ jsx7("div", { className: "hsk-cb-src-imgwrap-empty", children: /* @__PURE__ */ jsx7(SparkleIcon3, {}) }),
...
function ChatModal({
  ...
  const [loadingMessageIndex, setLoadingMessageIndex] = useState8(0);
  ...
  const [phoneInput, setPhoneInput] = useState8(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("huskel_user_phone") || "";
    }
    return "";
  });
  const [merchantRef, setMerchantRef] = useState8(null);
  const [paymentPhase, setPaymentPhase] = useState8("idle");
  const [sessions, setSessions] = useState8(() => loadSessions());
  const [sidebarOpen, setSidebarOpen] = useState8(false);
  const [replayMessages, setReplayMessages] = useState8(null);
      ...
      children: /* @__PURE__ */ jsxs5("div", { className: cn("hsk-cb-panel hsk-cb-panel--with-sidebar", classNames.panel), onClick: (e) => e.stopPropagation(), children: [
        ...
        /* @__PURE__ */ jsxs5("div", { className: "hsk-cb-main", children: [
          ...
          /* @__PURE__ */ jsxs5("div", { className: "hsk-cb-msgs", ref: msgsContainerRef, children: [
            ...
            loading && !streaming && /* @__PURE__ */ jsxs5("div", { className: "hsk-cb-typing-row", style: { display: "flex", alignItems: "flex-start", gap: "10px" }, children: [
              /* @__PURE__ */ jsx7("div", { className: "hsk-cb-ai-icon", style: { display: "flex", alignItems: "center", marginTop: "4px" }, children: /* @__PURE__ */ jsx7(SparkleIcon3, {}) }),
              /* @__PURE__ */ jsxs5("div", { style: { display: "flex", flexDirection: "column", gap: "6px" }, children: [
                ...
                /* @__PURE__ */ jsxs5("div", { className: "hsk-cb-typing", style: { margin: 0, alignSelf: "flex-start" }, children: [
...
function CheckoutModal({
  ...
  const [loadingConfig, setLoadingConfig] = useState9(true);
  ...
  const [firstName, setFirstName] = useState9(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("huskel_user_firstname") || "";
    }
    return "";
  });
  const [lastName, setLastName] = useState9(() => {
    if (typeof window !== "undefined") {
  ...
  const [payError, setPayError] = useState9(null);
...
//# sourceMappingURL=chunk-YTSTOLFZ.mjs.map