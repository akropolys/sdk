...
function CartContextCard({ cart, defaultCurrency }: CartContextCardProps) {
  ...
  const total = cart.total ?? cart.items.reduce((s: number, i: CartItem) => s + i.price_numeric * i.quantity, 0);
...
function ActionPills({ cart, actionType, onSend, loading }: ActionPillsProps) {
  ...
  if (hasItems && lastItem) {
    if (actionType === 'add_to_cart') {
      ...
      pills.push({ emoji: '🗑️', label: `Remove ${shortName}`, query: `Remove the ${name} from my cart` });
    }
...
function SourcesCarousel({ sources, defaultCurrency, onSelectSource, referencedIds = [] }: SourcesCarouselProps) {
...
function ChatModal({
  ...
  const client = useAkropolysContext();
  const { messages, sources, loading, streaming, error, lastAction, lastIntent, send, reset, referencedIds } = useKiku();
  ...
  const handleSend = async (text?: string, extraAttachments?: ChatAttachment[]) => {
    ...
    setAttachments([]);
    await send(q, undefined, toSend.length > 0 ? toSend : undefined);
  };
                  ...
                  <div key={idx} className="hsk-cb-msg-group">
                    {isUser ? (
                      <div className="hsk-cb-user-msg">
                        ...
                        {msg.content && <div className="hsk-cb-user-bubble">{msg.content}</div>}
                      </div>
                    ) : (
